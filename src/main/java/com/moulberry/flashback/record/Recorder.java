...
public class Recorder {
    ...
    private final Queue<Packet<? super ClientGamePacketListener>> pendingPackets = new ConcurrentLinkedQueue<>();
    ...
    public void endTick(boolean close) {
        ...
        if (minecraft.level != null) {
            boolean finished = this.finishedPausing;
    ...
    public void writeLevelEvent(int type, BlockPos blockPos, int data, boolean globalEvent) {
        ...
        this.pendingPackets.add(new ClientboundLevelEventPacket(type, blockPos, data, globalEvent));
    }
    ...
    public void writeSound(Holder<SoundEvent> holder, SoundSource soundSource, double x, double y, double z, float volume, float pitch, long seed) {
        ...
        this.pendingPackets.add(new ClientboundSoundPacket(holder, soundSource, x, y, z, volume, pitch, seed));
    }
    ...
    public void writeEntitySound(Holder<SoundEvent> holder, SoundSource soundSource, Entity entity, float volume, float pitch, long seed) {
        ...
        this.pendingPackets.add(new ClientboundSoundEntityPacket(holder, soundSource, entity, volume, pitch, seed));
    }
    ...
    public void writePacketAsync(Packet<?> packet, ConnectionProtocol phase) {
        ...
        this.pendingPackets.add((Packet<? super ClientGamePacketListener>) packet);
    }
    ...
    public void writeSnapshot(boolean asActualSnapshot) {
        ...
        var loginPacket = new ClientboundLoginPacket(localPlayer.getId(), level.getLevelData().isHardcore(), gameMode.getPlayerMode(),
            gameMode.getPreviousPlayerMode(), connection.levels(), connection.registryAccess().freeze(),
            level.dimensionTypeRegistration().unwrapKey().orElseThrow(), level.dimension(), hashedSeed, 1,
            minecraft.options.getEffectiveRenderDistance(), level.getServerSimulationDistance(), localPlayer.isReducedDebugInfo(),
            localPlayer.shouldShowDeathScreen(), level.isDebug(), level.getLevelData().isFlat, Optional.empty(), 0);
        gamePackets.add(loginPacket);
        gamePackets.add(new ClientboundUpdateEnabledFeaturesPacket(FeatureFlags.REGISTRY.toNames(connection.enabledFeatures())));
        gamePackets.add(new ClientboundUpdateTagsPacket(serializedTags));
